...
export async function searchRooms(filters) {
  const { type, destination, guestsNum, bedroomsNum, bedsNum, price } = await filters;
...
export async function getHomelyRooms(userId) {
  ...
  export async function getHomelyBookings(userId) {
    const { data, error } = await supabase
      .from("reservations")
      .select(
        "*, rooms!inner(user_id,country, city, room_images, room_name)"
      )
  ...
  export async function getHomelyBookingsInfo(reservationId) {
    const { data, error } = await supabase
      .from("reservations")
      .select(
        "*, rooms!inner(user_id,country, city,address,guests_num, beds_num,bedrooms_num, bathrooms_num, room_images, room_name)"
      )
  ...
  export async function getHostingNumber(userId) {
    const { data, error } = await supabase
    .from('reservations')
    .select(`
      *,
 ...
 export async function deleteRoom(roomId) {
  try {
    ...
    revalidatePath("/account/homely-rooms","layout");
    return true;
  } catch (error) {
    return false;
  }
...
{/* wishList */}
  ...
  export async function getWishRooms(userId) {
    let { data, error } = await supabase
      ...
      .select("*")
      .eq("user_id", userId)
      .select(`id ,rooms(country, city, room_images, room_name, room_price, address)`);
    if (error) {
      throw new Error(error.message);
    }
    return data;
...
{/* reservations */}
  export async function getReservations(userId) {
    let { data, error } = await supabase
      ...
      .eq("user_id", userId)
      .select(
        "*,rooms(country, city, room_images, room_name)"
      )
      .order("check_in");
    if (error) {
      throw new Error(error.message);
